...
import logging # <<< FIX #1: Import the logging module >>>
...
# --- Pong Game Setup ---
app = Dash(__name__)
app.title = "SSVEP BCI Pong (Weighted All-Channel)"

def get_initial_game_state():
    return {'player_x': GAME_WIDTH / 2, 'ai_x': GAME_WIDTH / 2, 'ball_x': GAME_WIDTH / 2, 'ball_y': GAME_HEIGHT / 2, 'ball_vx': 0, 'ball_vy': INITIAL_BALL_SPEED_Y, 'player_score': 0, 'ai_score': 0, 'active_channel': 0}

app.layout = html.Div(style={'backgroundColor': '#111111', 'color': '#DDDDDD', 'textAlign': 'center', 'fontFamily': 'monospace'}, children=[
    html.H1("SSVEP BCI Pong (Weighted All-Channel)"),
...
def update_game(n, state):
    ...
    channel_metrics = []; channel_weights = []
    with buffer_lock:
        for ch in range(ADS1299_NUM_CHANNELS):
            ...
            metric = psd[idx_right] - psd[idx_left]; weight = abs(metric)
            channel_metrics.append(metric); channel_weights.append(weight)

    if sum(channel_weights) > 1e-9:
        weighted_metric = np.average(channel_metrics, weights=channel_weights)
        state['active_channel'] = np.argmax(channel_weights) + 1
    else:
        weighted_metric = 0; state['active_channel'] = 'None'

    control_metric_history.append(weighted_metric)
    ...
    if state['ball_y'] < -BALL_RADIUS or state['ball_y'] > GAME_HEIGHT + BALL_RADIUS:
        ...
        p_score, a_score, b_chan = state['player_score'], state['ai_score'], state['active_channel']
        state = get_initial_game_state(); state.update({'player_score': p_score, 'ai_score': a_score, 'active_channel': b_chan})
    ...
    # <<< FIX #2: Explicitly set the x and y coordinates for the heatmap >>>
    # This tells the heatmap to stretch across the entire game area.
    fig.add_trace(go.Heatmap(
        z=gradient_z_values,
        x=np.linspace(0, GAME_WIDTH, GRADIENT_X_RESOLUTION),
        y=np.linspace(0, GAME_HEIGHT, GRADIENT_Y_RESOLUTION),
    ...
    focus_text = f"Strongest Channel: {state.get('active_channel', 'N/A')} | Control Signal: {control_signal:.2f}"
...
def main():
    ...
    if serial_port_object:
        ...
        # <<< FIX #3: Silence the default web server logs >>>
