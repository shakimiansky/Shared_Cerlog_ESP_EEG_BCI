...
PADDLE_MAX_SPEED = 40
PADDLE_ACCELERATION_FACTOR = 0.8
INITIAL_BALL_SPEED_Y = -5
BALL_SPEEDUP_ON_HIT = 1.03
MAX_BALL_SPEED_Y = 15
AI_PADDLE_SPEED = 5
AI_MISTAKE_CHANCE = 0.05
BALL_SPIN_FACTOR = 0.07
...
# --- Control Logic ---
# <<< CHANGE: Sensitivity (amplifier) set to 5.0 as requested >>>
BCI_SCORE_AMPLIFIER = 5.0
CONTROL_DEAD_ZONE = 0.2
CONTROL_SENSITIVITY_EXP = 2.0
EMA_SMOOTHING_FACTOR = 0.6
BCI_SCORE_NORMALIZATION_CAP = 1.0
...
notch_b, notch_a = iirnotch(NOTCH_FREQ_HZ, NOTCH_QUALITY_FACTOR, SAMPLING_RATE_HZ)
...
def update_game_physics(_, state, bci_command, app_status, key_data):
    ...
    if key_command == 'a':
        target_vx = -PADDLE_MAX_SPEED
    elif key_command == 'd':
        target_vx = PADDLE_MAX_SPEED
    else:
        score = bci_command.get('smoothed_score', 0.0)
        
        if abs(score) < CONTROL_DEAD_ZONE:
            final_control_signal = 0.0
        else:
            normalized_score = score / BCI_SCORE_NORMALIZATION_CAP
            clipped_score = np.clip(normalized_score, -1.0, 1.0)
            final_control_signal = np.sign(clipped_score) * (abs(clipped_score) ** CONTROL_SENSITIVITY_EXP)
        
        target_vx = PADDLE_MAX_SPEED * final_control_signal

    state['player_vx'] = (state.get('player_vx', 0) * (1 - PADDLE_ACCELERATION_FACTOR)) + (target_vx * PADDLE_ACCELERATION_FACTOR)
    ...
    if state['ball_vy'] > 0 and state['ball_y'] + BALL_RADIUS >= GAME_HEIGHT - PADDLE_HEIGHT:
        if abs(state['player_x'] - state['ball_x']) < PADDLE_WIDTH / 2 + BALL_RADIUS:
            state['ball_vy'] = min(abs(state['ball_vy']) * BALL_SPEEDUP_ON_HIT, MAX_BALL_SPEED_Y) * -1
    ...
    if state['ball_vy'] < 0 and state['ball_y'] - BALL_RADIUS <= PADDLE_HEIGHT:
        if abs(state['ai_x'] - state['ball_x']) < PADDLE_WIDTH / 2 + BALL_RADIUS:
            # <<< FIX: Corrected AI paddle physics >>>
            # The original buggy code calculated a new NEGATIVE velocity.
            # This version correctly calculates a POSITIVE (downward) velocity.
            state['ball_vy'] = min(abs(state['ball_vy']) * BALL_SPEEDUP_ON_HIT, MAX_BALL_SPEED_Y)
    ...
    if point_scored:
        ...
        state['ball_vx'] = 0
        state['ball_vy'] = INITIAL_BALL_SPEED_Y
        state['player_x'] = GAME_WIDTH / 2
        state['player_vx'] = 0
        state['ai_x'] = GAME_WIDTH / 2
...
def manage_app_flow(status_n, pause_clicks, restart_clicks, app_status, cal_data):
    ...
    if triggered_id == 'restart-button' and restart_clicks > 0:
        ...
        if status == 'STARTING':
            ...
            final_thresh_left = min(threshold_left, -MIN_THRESHOLD_GAP/2)
            final_thresh_right = max(threshold_right, MIN_THRESHOLD_GAP/2)
            cal_data['thresholds'] = {'left': final_thresh_left, 'right': final_thresh_right}
            ...
            print(f"--> FINAL THRESHOLDS: Left < {final_thresh_left:.3f} | Right > {final_thresh_right:.3f}")
            if final_thresh_right - final_thresh_left < MIN_THRESHOLD_GAP:
                 print("\n*** WARNING: Calibration may be inaccurate. Thresholds are very close. Check electrode connection and focus. ***\n")
...
def update_feedback_plots(_, bci_command, cal_data):
    ...
    max_range = BCI_SCORE_NORMALIZATION_CAP * BCI_SCORE_AMPLIFIER * 1.2
        
    control_fig = go.Figure(layout=go.Layout(title='Live BCI Control Score', template='plotly_dark', yaxis=dict(range=[-max_range, max_range])))
    ...
    if thresholds:
        ...
        control_fig.add_hrect(y0=-CONTROL_DEAD_ZONE, y1=CONTROL_DEAD_ZONE, fillcolor="grey", opacity=0.3, layer="below", line_width=0, annotation_text="Dead Zone")
...
def main():
    ...
    if serial_port_object:
        ...
        print("\nDash server is running. Open http://1227.0.0.1:8050/ in your browser.")
